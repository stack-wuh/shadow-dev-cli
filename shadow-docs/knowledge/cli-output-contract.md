...
- 错误 code 与 `data.nextStep` 模板永不本地化；语言链固定为 `--lang` > `SHADOW_DEV_LANG` > locale 探测 > 默认 zh，且只影响 stderr 文案。**测试对 stderr 文案的断言必须显式钉 `--lang` 或 `SHADOW_DEV_LANG`**——CI runner 的 locale（macos/windows 为 en）会经探测改变渲染语言，隐式依赖机器环境的断言在非中文 runner 上恒假。
