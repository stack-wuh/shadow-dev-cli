...
const UI = {
  ...
  en: {
    ...
    version: 'ℹ shadow-dev-cli version {v}',
...
export const HINTS = {
  ...
  NAME_REQUIRED: { zh: '缺少必填参数（详见下列参数行与示例）', en: 'missing required argument (see flag lines below)' },
  ...
  WORKFLOW_NOT_INSTALLED: { zh: 'workflow 产物未安装；先运行 workflow plan + workflow execute', en: 'workflow artifact not installed; run workflow plan + workflow execute first' },
  ARTIFACT_INVALID: { zh: '产物缺少 marketplace.json/package.json/skills 契约文件', en: 'artifact is missing its marketplace.json/package.json/skills contract' },
  RELEASE_NOT_FOUND: { zh: '找不到 release 或其 tarball 资产（--release 可固定 tag）', en: 'release or its tarball asset not found (pin one with --release)' },
  DOWNLOAD_FAILED: { zh: 'release 资产下载失败（网络或 URL 形态异常）', en: 'release asset download failed (network or malformed url)' },
  NO_PREVIOUS: { zh: '没有可回滚的上一版本', en: 'no previous version available for rollback' },
  NOT_LINKED: { zh: '当前无 LINK 指针；先运行 workflow link --dir <path>', en: 'no LINK pointer; run workflow link --dir <path> first' },
  DIR_REQUIRED: { zh: '缺少 --dir（产物目录）', en: '--dir (artifact directory) is required' },
  HOST_NOT_FOUND: { zh: '没有匹配的宿主适配器，或其 skills 目录在本机不存在', en: 'no matching host adapter, or its skills dir is absent on this machine' },
  HOST_REQUIRED: { zh: '缺少 --host（unbind 需要显式宿主名）', en: '--host (explicit host name) is required for unbind' },
  UNMANAGED_TARGET: { zh: '同名目录非本工具托管，拒绝覆盖；手动移除后重试', en: 'target dirs are not managed by this tool; remove them manually first' },
  ADAPTERS_MISSING: { zh: '产物缺 adapters/（需要随产物分发描述符的 workflow 版本）', en: 'artifact lacks adapters/ (need a workflow release shipping descriptors)' },
  NOTHING_TO_UNBIND: { zh: '该宿主没有可解绑的托管 skills', en: 'no managed skills to unbind for this host' },
}
