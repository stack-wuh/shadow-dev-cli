...
export function confirm(o, ph = false) {
  if (!o.confirm) throw err('CONFIRMATION_REQUIRED', 'CONFIRMATION_REQUIRED', 2)
  if (ph && !o['plan-hash']) throw err('PLAN_HASH_REQUIRED', 'PLAN_HASH_REQUIRED', 2)
}

export function name(o) {
  if (!o.name) throw err('NAME_REQUIRED')
  return o.name
}
...
export function readBody(o, n) {
  ...
  const p = o['body-file']
  if (!existsSync(p)) throw err('BODY_FILE_NOT_FOUND')
